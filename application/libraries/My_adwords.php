...
function GetCampaign($account_id) {
  // Get the service, which loads the required classes.
  $campaignService = $this->GetService('CampaignService', ADWORDS_VERSION);
  ...
  $selector = new Selector();
  $selector->fields =
      array('Id', 'Name', 'Status', 'ServingStatus', 'StartDate', 'EndDate');
  // Set date range to request stats for.
  $dateRange = new DateRange();
  $dateRange->min = date('Ymd', strtotime('-2 day'));
...
function GetCampaignStatsExample()
{
  // Get the service, which loads the required classes.
  $campaignService = $this->GetService('CampaignService', ADWORDS_VERSION);
  // Create selector.
          ...
          if (isset($page->entries)) {
            foreach($page->entries as $adGroup) {
              ...
              do {
                ...
                if (isset($page->entries)) {
                  foreach($page->entries as $adGroupAd) {
                    $varavara = urldecode($adGroupAd->ad->url);
                    $arr = explode('url[]=', $varavara);
                    $important = $arr[1];
                  }
                }
                else {
...
$dateRange = new DateRange();
 ...
 function GetAccountHierarchyExample() {
  // Get the service, which loads the required classes.
  $managedCustomerService =
      $this->GetService('ManagedCustomerService', ADWORDS_VERSION);
  ...
  //var_dump($graph);
//var_dump($graph->entries[0]->customerId);
  $customerList = array();
